...
class Main:
    def __init__(self):
        ...
        self.NPERIODS = 2500                                                     # number of periods                                           
        self.TIME_HIGH = 5                                                       # 5s pulse
        self.TIME_LOW = 0.005                                                     # send every 5ms
        self.PERIOD = self.TIME_HIGH + self.TIME_LOW
        self.onset_times = [ (self.PERIOD * i) for i in range(self.NPERIODS) ]
        for n in range(0, self.NPERIODS):
            self.rise = [True, True, True]
            if n <= self.NPERIODS:
                self.fall = [False, False, True]
            elif n > self.NPERIODS:
    ...
    def config_DLCPRO(self):
        ...
        self.DLC = DLCpro(SerialConnection(DLCPRO_CONNECTION)) #PARAMS!!!
        
    def trig_DLCPRO(self):
        
        self.DLC.laser1.wide_scan.trigger.input_channel:0 #TRIGG!!
    ...
    def trigger_mea(self):
                ...
                with nidaqmx.Task() as task:
                    ...
                    task.do_channels.add_do_chan("cDAQ1Mod4/port0/line3")               # DIO2: Gate17, Toptica DLC pro
